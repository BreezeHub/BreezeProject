...
namespace NTumbleBit.Tests
{
    public class PuzzleProtocolsTests
    {
        ...
        public void TestPuzzlePromise()
        {
            ...
            var client = new PromiseClientSession(Network.Main, parameters);
            ...
            var resigned = redeemTransaction.ReSign(escrow, Network.Main);
            TransactionBuilder bb = new TransactionBuilder(Network.Main);
            bb.AddCoins(server.GetInternalState().EscrowedCoin);
            Assert.True(bb.Verify(resigned));

            //Check can ve reclaimed if malleated
            bb = new TransactionBuilder(Network.Main);
            escrow.Outpoint = new OutPoint(escrow.Outpoint.Hash, 10);
            bb.AddCoins(escrow);
            resigned = redeemTransaction.ReSign(escrow, Network.Main);
        ...
        public void TestPuzzleSolver()
        {
            ...
            var resigned = offerTransaction.ReSign(client.EscrowedCoin, Network.Main);

            TransactionBuilder txBuilder = new TransactionBuilder(Network.Main);
            txBuilder.AddCoins(client.EscrowedCoin);
            Assert.True(txBuilder.Verify(resigned));

            resigned = fulfill.ReSign(offerCoin, Network.Main, out bool cached);
            Assert.False(cached);
            txBuilder = new TransactionBuilder(Network.Main);
            txBuilder.AddCoins(offerCoin);
            Assert.True(txBuilder.Verify(resigned));

            //Test again to see if cached signature works well
            resigned = fulfill.ReSign(offerCoin, Network.Main, out cached);
            Assert.True(cached);
            Assert.True(txBuilder.Verify(resigned));

            var offerRedeemTx = offerRedeem.ReSign(offerCoin, Network.Main);
            txBuilder = new TransactionBuilder(Network.Main);
            txBuilder.AddCoins(offerCoin);
            ...
            var escapeTransaction =
                server.GetSignedEscapeTransaction(clientEscapeSignature, FeeRate, new Key().ScriptPubKey);
        ...
        private void RoundTrip(ref SolverServerSession server, SolverParameters parameters, RsaKey key)
        {
            ...
            server = new SolverServerSession(key, parameters, clone, Network.Main);
        }
        ...
        private void RoundTrip(ref SolverClientSession client, SolverParameters parameters)
        {
            ...
            client = new SolverClientSession(parameters, clone, Network.Main);
        }
        ...
        {
            var clone = Serializer.Clone(client.GetInternalState());
            client = new PromiseClientSession(Network.Main, parameters, clone);
        }
