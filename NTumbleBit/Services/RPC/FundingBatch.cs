...
namespace NTumbleBit.Services.RPC
{
    class FundingBatch : BatchBase<TxOut, Transaction>
    {
        ...
        protected override async Task<Transaction> RunAsync(TxOut[] data)
        {
            ...
            var result = await _RPCClient.SendCommandAsync("signrawtransaction", response.Transaction.ToHex())
                .ConfigureAwait(false);

            return Transaction.Parse(((JObject) result.Result)["hex"].Value<string>());
        }
    }
}