...
namespace NTumbleBit.PuzzleSolver
{
    ...
    public class SolverServerSession : EscrowReceiver
    {
        public class SolvedPuzzle
        {
            ...
            public PuzzleValue Puzzle { get; set; }
            public SolutionKey SolutionKey { get; set; }
            public PuzzleSolution Solution { get; set; }

            public byte[] EncryptedSolution { get; set; }
        ...
        {
            public SolverServerStates Status { get; set; }

            public SolvedPuzzle[] SolvedPuzzles { get; set; }
            public Key FulfillKey { get; set; }
            public TransactionSignature OfferClientSignature { get; set; }
            public int ETag { get; set; }
            public ScriptCoin OfferCoin { get; set; }
        ...
        public SolverServerSession(RsaKey serverKey, Network network) : this(serverKey, null, network)
        {
        }

        public SolverServerSession(RsaKey serverKey, SolverParameters parameters, Network network)
        {
            ...
            if (serverKey.PubKey != parameters.ServerKey)
                throw new ArgumentNullException(
                    $"Private key not matching expected public key: {nameof(serverKey.PubKey)} != {nameof(parameters.ServerKey)}");
        ...
        public SolverServerSession(RsaKey serverKey, SolverParameters parameters, State state, Network network)
            : this(serverKey, parameters, network)
        {
            if (state == null)
                return;
            InternalState = state;
        ...
        public ServerCommitment[] EndSolvePuzzles()
        {
            ...
            foreach (var solved in InternalState.SolvedPuzzles)
            {
                ...
                commitments.Add(new ServerCommitment(solved.SolutionKey.GetHash(),
                    solved.EncryptedSolution ?? solved.GetEncryptedSolution()));
            }
        ...
        {
            var offerTransaction = GetUnsignedOfferTransaction();
            return offerTransaction.SignInput(_Network, InternalState.EscrowKey, InternalState.EscrowedCoin);
        }
            ...
            FeeRate feeRate)
        {
            ...
            if (!offer.Inputs.AsIndexedInputs().First().VerifyScript(Network.Main, InternalState.EscrowedCoin))
        ...
        private PubKey AssertValidSignature(TransactionSignature clientSignature, Transaction offer)
        {
            ...
            var coin = InternalState.EscrowedCoin.Clone(_Network);
            coin.OverrideScriptCode(escrow.GetInitiatorScriptCode());
            var signedHash = offer.Inputs.AsIndexedInputs().First()
                .GetSignatureHash(_Network, coin, clientSignature.SigHash);
        ...
        public Transaction GetSignedEscapeTransaction(TransactionSignature clientSignature, FeeRate feeRate,
            Script cashout)
        {
            AssertState(SolverServerStates.WaitingEscape);
