...
namespace NTumbleBit.ClassicTumbler.Server.Controllers
{
    public class MainController : Controller
    {
        ...
        {
            get { return _Runtime.ClassicTumblerParameters; }
        }
            ...
            [FromBody] SignVoucherRequest request)
        {
            ...
            if (request.MerkleProof.PartialMerkleTree
                    ...
                    .FirstOrDefault() != request.Transaction.GetHash() ||
                !request.MerkleProof.Header.CheckProofOfWork(this.Parameters.Network.Consensus))
            {
            ...
            var confirmations =
                Services.BlockExplorerService.GetBlockConfirmations(request.MerkleProof.Header.GetHash());
            ...
            var expectedEscrow =
                new EscrowScriptPubKeyParameters(request.ClientEscrowKey, key.PubKey, cycle.GetClientLockTime());

            var expectedTxOut = new TxOut(Parameters.Denomination + Parameters.Fee,
                expectedEscrow.ToScript().WitHash.ScriptPubKey.Hash);
            var escrowedCoin =
                transaction
                    .Outputs
                    .AsCoins()
            ...
            QueueWork(async () =>
            {
                try
                {
                    if (!await Services.BlockExplorerService.TrackPrunedTransactionAsync(request.Transaction,
                        request.MerkleProof))
                    {
                    ...
                    Tracker.AddressCreated(cycle.Start, TransactionType.ClientEscrow, escrowedCoin.ScriptPubKey,
                        correlation);
                    Tracker.TransactionCreated(cycle.Start, TransactionType.ClientEscrow, request.Transaction.GetHash(),
                        correlation);
                    var solution = request.UnsignedVoucher.WithRsaKey(Runtime.VoucherKey.PubKey)
                        .Solve(Runtime.VoucherKey);
                    Repository.SaveSignedVoucher(cycle.Start, request.ChannelId, solution);
                    Logs.Tumbler.LogInformation($"Cycle {cycle.Start} Proof of Escrow signed for " +
                                                transaction.GetHash());
                }
                catch (Exception ex)
                {
            ...
            [FromBody] OpenChannelRequest request)
        {
            ...
            try
            {
                if (!Parameters.VoucherKey.PublicKey.Verify(request.Signature,
                    NBitcoin.Utils.ToBytes((uint) request.CycleStart, true), request.Nonce))
                ...
                var unused = Services.WalletService.FundTransactionAsync(txOut, fee)
                    .ContinueWith(async (Task<Transaction> task) =>
                    {
                        try
                        {
                            ...
                            if (Repository.MarkUsedNonce(cycle.Start,
                                new uint160(tx.GetHash().ToBytes().Take(20).ToArray())))
                            {
                                var bobCount = Parameters.CountEscrows(tx, Client.Identity.Bob);
                                Logs.Tumbler.LogInformation(
                                    $"Cycle {cycle.Start} channel created {tx.GetHash()} with {bobCount} users");
                            ...
                            Tracker.AddressCreated(cycle.Start, TransactionType.TumblerEscrow, txOut.ScriptPubKey,
                                correlation);
                            var coin = tx.Outputs.AsCoins().First(o =>
                                o.ScriptPubKey == txOut.ScriptPubKey && o.TxOut.Value == txOut.Value);
                            ...
                            var escrowedCoin = coin.ToScriptCoin(escrow.ToScript());
                            Logs.Tumbler.LogDebug($"BeginOpenChannel - channelId : {channelId}, escrowedCoin.Outpoint.Hash : {escrowedCoin.Outpoint.Hash}, escrowedCoin.Outpoint.N : {escrowedCoin.Outpoint.N}, escrowKey.GetWif() : {escrowKey.GetWif(Parameters.Network)}, redeemDestination : {redeem.ScriptPubKey}");
                            session.ConfigureEscrowedCoin(channelId, escrowedCoin, escrowKey, redeem.ScriptPubKey);

                            var redeemTx = session.CreateRedeemTransaction(fee);
                            Services.TrustedBroadcastService.Broadcast(cycle.Start, TransactionType.TumblerRedeem,
                                correlation, redeemTx);
            ...
            uint160 channelId)
        {
            ...
            var session =
                GetPromiseServerSession(cycle.Start, channelId, CyclePhase.TumblerChannelEstablishment, false);
            if (session == null)
                return null;
            ...
            [FromBody] SignatureWrapper wrapper)
        {
            ...
            var unused = Runtime.Services.WalletService
                .ReceiveAsync(state.EscrowedCoin, clientSignature, state.EscrowKey, fee)
                .ContinueWith(async (Task<Transaction> task) =>
                {
                    try
                    {
                        ...
                        var correlation = GetCorrelation(session);
                        Tracker.AddressCreated(cycleId, TransactionType.ClientEscape, tx.Outputs[0].ScriptPubKey,
                            correlation);
